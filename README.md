...
A minimal, always-current VS Code sidebar view of your recent LangFuse traces. No backend, no other vendor, no floating window — one list of recent calls (model and cost inline), one summary strip, and click-to-expand details for each call, with an optional link to LangFuse's own UI for deep debugging.
